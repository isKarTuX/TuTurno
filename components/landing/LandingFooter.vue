...
const footerLinks = {
  producto: [
    ...
    { label: 'Para entidades', to: '/#para-entidades' },
    { label: 'Para operadores', to: '/#para-operadores' },
...
const socialLinks = [
  ...
  {
    label: 'WhatsApp',
    icon: 'M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.06 6.988 2.754a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z',
    href: '#',
  },
  {
    label: 'Instagram',
...
const contactInfo = {
  ...
  emailSoporte: 'soporte@tuturno.co',
...
<template>
  <footer class="relative overflow-hidden">
    <div class="absolute inset-0 bg-gradient-to-b from-[--bg-base] via-[--bg-surface] to-[--bg-base]" />
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
        <div class="lg:col-span-4">
          ...
          <p class="text-sm text-[--text-secondary] mb-6 max-w-sm leading-relaxed">
            Eliminando las filas en EPS, bancos y oficinas públicas de Colombia.
          ...
          <div class="space-y-3 mb-8">
            <a
              ...
              class="flex items-center gap-3 text-sm text-[--text-muted] hover:text-white transition-colors group"
            >
            ...
            <div class="flex items-center gap-3 text-sm text-[--text-muted] group">
          ...
          <div class="flex items-center gap-3">
            <a
              ...
              class="w-10 h-10 rounded-xl bg-white/5 hover:bg-white/10 border border-white/5 hover:border-white/10 flex items-center justify-center text-[--text-muted] hover:text-white transition-all"
            >
              <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                <path :d="social.icon" />
              </svg>
        ...
        <div class="lg:col-span-8 grid grid-cols-2 md:grid-cols-4 gap-8">
          <div>
            <h4 class="text-sm font-semibold text-white mb-5">Producto</h4>
            <ul class="space-y-3">
          ...
          <div>
            <h4 class="text-sm font-semibold text-white mb-5">Recursos</h4>
            <ul class="space-y-3">
              <li v-for="link in footerLinks.recursos" :key="link.label">
                <NuxtLink
                  :to="link.to"
                  class="text-sm text-[--text-muted] hover:text-white transition-colors"
                >
          ...
          <div>
            <h4 class="text-sm font-semibold text-white mb-5">Empresa</h4>
            <ul class="space-y-3">
          ...
          <div>
            <h4 class="text-sm font-semibold text-white mb-5">Legal</h4>
            <ul class="space-y-3">
      ...
      <div class="mt-16 pt-8 border-t border-white/5">
        <div class="flex flex-col md:flex-row items-center justify-between gap-4">
          <div class="flex flex-col sm:flex-row items-center gap-4 text-sm text-[--text-muted]">
            <p>&copy; {{ currentYear }} TuTurno. Universidad de Córdoba.</p>
          ...
          <div class="flex items-center gap-2">
            ...
            <span class="text-xs text-[--text-muted]">Todos los sistemas operativos</span>
          </div>
        </div>
      </div>
    </div>
