...
const entitiesBenefits = [
  {
    ...
    description: 'Ve cuántos turnos esperan, tiempos promedio y métricas clave de atención.',
    icon: 'chart',
  },
  {
    title: 'Optimiza tu operación',
    description: 'Reduce tiempos de espera y mejora la satisfacción de tus clientes.',
    icon: 'efficiency',
  },
  {
  ...
  },
  {
    ...
    description: 'Conecta con tus sistemas existentes. API文档 completa disponible.',
    icon: 'plug',
  },
]
  ...
  <section class="py-24 lg:py-32 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto relative">
      ...
      <Transition name="tab-fade" mode="out-in">
        <div v-if="activeTab === 'citizens'" key="citizens" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div
            v-for="(benefit, index) in citizensBenefits"
            :key="benefit.title"
            class="group relative p-6 lg:p-8 rounded-2xl bg-surface border border-white/6 hover:border-primary/30 transition-all duration-300 hover:-translate-y-1"
          ...
          >
            ...
            <div class="relative">
              <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-primary/20 to-primary/10 border border-primary/20 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300">
                <svg class="w-7 h-7 text-primary-light" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                  <path stroke-linecap="round" stroke-linejoin="round" :d="iconPaths[benefit.icon]" />
                </svg>
              </div>

              <h3 class="text-lg font-semibold text-white mb-3 group-hover:text-primary-light transition-colors">
                {{ benefit.title }}
              </h3>
            ...
            <div class="absolute bottom-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-primary/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
          </div>
        </div>

        <div v-else-if="activeTab === 'entities'" key="entities" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div
        ...
        <div v-else-if="activeTab === 'operators'" key="operators" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div
      ...
      <div class="flex flex-col sm:flex-row gap-4 justify-center mt-12">
        ...
        <NuxtLink
          to="/auth/login"
          class="group relative inline-flex items-center justify-center gap-2.5 px-8 py-4 text-sm font-semibold text-white rounded-xl border border-white/10 hover:border-white/20 hover:bg-white/5 transition-all duration-300"
        >
